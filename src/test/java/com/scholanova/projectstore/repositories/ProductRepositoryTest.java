...
public class ProductRepositoryTest {
    ...
    private void insertStore(Store s) {
		String query = "INSERT INTO STORES (ID,NAME) VALUES ('%d','%s') ";

        jdbcTemplate.execute(
                String.format(query, s.getId(), s.getName())
                );
    }
    ...
    class Test_Sum {
    	@Test
    	void givenAStoreWithProductsThenSumOfValuesIsTheRightAmount() throws Exception {
            // Given
    		Store store = new Store(1,"Auchan");
    		Product product1 = new Product(2, "pizza", "surgele",270,1);
            Product product2 = new Product(3, "pomme","fruit",90,1);
            
            insertProduct(product1);
            insertProduct(product2);
            
            // When
            Integer sumProducts = productRepository.getStoreSum(store.getId());
            // Then
            assertThat(sumProducts).isEqualTo(360);
        }
    	
    	@Test
    	void givenAStoreWithProductsThenCountOfItemsIsTheRightAmount() throws Exception {
            // Given
    		Store store = new Store(1,"Auchan");
    		insertStore(store);
    		Product product1 = new Product(2, "pizza", "surgele",270,1);
            ...
            // When
            List<Product> products = productRepository.getProducts(store.getId());
            // Then
    ...
    class Test_delete {
    	@Test
    	void givenAStoreWithTwoProducts_WhenDeleteOne_ThenRemainsOne() throws Exception {
    		//GIVEN
    		Store store = new Store(1,"Auchan");
//    		insertStore(store);
    		Product product1 = new Product(2, "pizza", "surgele",270,1);
            ...
            //WHEN
            productRepository.deleteProduct(1,2);
            
            //THEN
            List<Product> products = productRepository.getProducts(store.getId());
            assertThat(products.size()).isEqualTo(1);
            assertThat(products.get(0).getId()).isEqualTo(3);
            assertThat(products.get(0).getName()).isEqualTo("pomme");
